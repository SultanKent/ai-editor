// components/services/IdeaGeneration.jsx
...
const IdeaGeneration = () => {
  ...
  const handleSubmit = async () => {
    ...
    const generateIdeaPrompt = `Based on the following text, generate a creative idea for a story, character, or plot twist. Please be as original and detailed as possible. обезательно ответить на том языке на которым ответили запрос Text: ${inputText}`;
    ...
    try {
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          model: 'gpt-4',
          messages: [{ role: 'user', content: generateIdeaPrompt }],
          max_tokens: 1000,
        }),
      });
    ...
    <div className="service-container">
      <div className="service-content">
        ...
        <div className="service-output">
          ...
          {generatedIdea && <p>{generatedIdea}</p>}
        </div>
      </div>
    </div>
  );
